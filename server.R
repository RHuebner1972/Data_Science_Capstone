...
# 4. If no Bigram is found, back off to the most common word with highest frequency 'the' is returned.
  ...
  if (length(xs)>= 3) {
    ...
    else {mesg <<- "Next word is predicted using quadgram."; head(quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4],1)}
  }
  else if (length(xs) == 2){
    xs <- tail(xs,2)
    ...
    else {mesg<<- "Next word is predicted using a trigram."; head(trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3],1)}
  }
  else if (length(xs) == 1){
    xs <- tail(xs,1)
    ...
    else {mesg <<- "Next word is predicted using bigram."; head(bigram[bigram$unigram == xs[1],2],1)}
  }
}
